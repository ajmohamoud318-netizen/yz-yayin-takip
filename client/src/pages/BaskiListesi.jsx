...
import { Printer, Factory, Ship, PackageCheck, Target, Trash2, Link as LinkIcon, Plus } from 'lucide-react'
...
import { cn, formatDateTr, formatTargetDate } from '@/lib/utils'
...
export default function BaskiListesi() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      ...
      <HedefProjelerSection />
    </div>
  )
}

/* ------------------------------------------------------------------ */
/*  Hedef Projeler — idea board                                        */
/* ------------------------------------------------------------------ */

/**
 * A shared scratchpad for book ideas that aren't real projects yet — often
 * just a link someone spotted on Instagram. Lives at the bottom of Baskı
 * Listesi rather than its own page since it's a lightweight, low-traffic
 * list, not a pipeline view. See server migration
 * 036__target_project_ideas.sql.
 */
function HedefProjelerSection() {
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">
            Hedef Projeler{ideas.length > 0 && `, ${ideas.length}`}
          </h2>
