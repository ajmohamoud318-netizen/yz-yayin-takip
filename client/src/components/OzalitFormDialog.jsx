...
const STORAGE_KEY   = (id)           => `yz_ozalit_form_${id}`
const SNAPSHOT_KEY  = (id, attempt)  => `yz_ozalit_form_${id}_snap_${attempt}`
...
function parseSaved(raw) {
  ...
  try {
    ...
    const form = {}
    const migrated = []
    for (const key of KNOWN_FIELDS) if (parsed[key] !== undefined) form[key] = parsed[key]
...
function loadSaved(id)             { return parseSaved(localStorage.getItem(STORAGE_KEY(id))) }
function loadSnapshot(id, attempt) { return parseSaved(localStorage.getItem(SNAPSHOT_KEY(id, attempt))) }

function saveForm(id, data, customRows, selectedComponents) {
...
function saveSnapshot(id, attempt, data, customRows, selectedComponents) {
  localStorage.setItem(SNAPSHOT_KEY(id, attempt), JSON.stringify({
    ...data,
    _customRows: customRows,
    _selectedComponents: selectedComponents ?? null,
  }))
...
function emptyForm(project, user) {
...
function openPrintWindow({ form, customRows, project, attemptNo, kind = 'ozalit' }) {
...
/** Opens one print window per selected component. Falls back to a single window. */
...
/**
 * Ozalit spec-sheet dialog.
 *
 * mode:
 *   'advance'  — printer forwards ozalit to approval queue (api.advanceProject)
 *   'approve'  — team leader approves and sends to production (api.approveProject)
 ...
 */
export default function OzalitFormDialog({ open, onOpenChange, project, mode = 'approve', onDone, viewAttempt }) {
  const { user } = useAuth()
  const { updateOne } = useProjectsStore()
  const [form, setForm] = useState(() => emptyForm(project, user))
  ...
  useEffect(() => {
    ...
    if (viewAttempt != null) {
      const snap = loadSnapshot(project.id, viewAttempt) ?? loadSaved(project.id)
      setForm(snap?.form ?? emptyForm(project, user))
  ...
  const attemptNo = viewAttempt ?? ((project?.ozalit_attempt ?? 0) + 1)
  ...
  function handleSave() {
    if (!project) return
    saveForm(project.id, form, customRows, selectedComponents)
    toast.success('Ozalit formu kaydedildi.')
    onOpenChange(false)
  }

  function handlePrint() {
    if (!project) return
    if (!readOnly) saveForm(project.id, form, customRows, selectedComponents)
    openMultiPrint({ form, customRows, project, attemptNo, kind: 'ozalit', selectedComponents })
  }
      ...
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto max-sm:left-0 max-sm:top-0 max-sm:h-screen max-sm:max-h-screen max-sm:w-screen max-sm:max-w-none max-sm:translate-x-0 max-sm:translate-y-0 max-sm:rounded-none">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Ozalit Üretim Formu
            {readOnly && <span className="ml-1 text-xs font-normal text-muted-foreground">({attemptNo}. Ozalit — salt görüntüleme)</span>}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="rounded-lg border bg-white">
          ...
          <div className="border-b px-4 py-2 text-right">
            <span className="text-sm font-bold">{attemptNo}. OZALİT</span>
          </div>
