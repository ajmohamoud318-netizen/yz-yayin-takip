...
import { FileText, Plus, Printer, Send, X, CheckSquare, Square } from 'lucide-react'
...
const STORAGE_KEY   = (id)           => `yz_demo_form_${id}`
const SNAPSHOT_KEY  = (id, attempt)  => `yz_demo_form_${id}_snap_${attempt}`
...
function openPrintWindow({ form, customRows, project, attemptNo, kind = 'demo' }) {
 ...
 */
export default function DemoFormDialog({ open, onOpenChange, project, mode = 'advance', onDone, viewAttempt }) {
  const { user } = useAuth()
  ...
  const attemptNo = viewAttempt ?? ((project?.demo_attempt ?? 0) + 1)
  ...
  function handleSave() {
    if (!project) return
    saveForm(project.id, form, customRows, selectedComponents)
    toast.success('Demo formu kaydedildi.')
    onOpenChange(false)
  }
  ...
  function handlePrint() {
    ...
    openMultiPrint({ form, customRows, project, attemptNo, kind: 'demo', selectedComponents })
  }
      ...
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Demo Üretim Formu
            {readOnly && <span className="ml-1 text-xs font-normal text-muted-foreground">({attemptNo}. Demo — salt görüntüleme)</span>}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="rounded-lg border bg-white">
          ...
          <div className="border-b px-4 py-2 text-right">
            <span className="text-sm font-bold">{attemptNo}. DEMO</span>
          </div>
