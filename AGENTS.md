...
- **A new `stage_history` event needs a row in `EVENTS` (`client/src/lib/project-history.js`).** Without one it falls through to the coarse `ACTIONS[action]` fallback, which for the common `action: 'system'` means: the generic label 'İlerletildi', no icon or tone of its own, and group `subtask` — so a matbaa change request counted toward the "Alt Görevler" filter chip, and the timeline printed the row's raw note as its heading (a change request reasoned "kaaa" became a row that said nothing). The row also declares `noteMode` when the server note either restates the label (`'echo'`) or already carries it (`'detail'`, every `subtask_*`).
